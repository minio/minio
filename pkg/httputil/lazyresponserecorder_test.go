...
// This code is imported from "https://golang.org/src/net/http/httptest/recorder_test.go",
// only recorder.go and record_test.go is imported and tweaked to return a ReadCloser for Body
package main
...
func TestRecorder(t *testing.T) {
	type checkFunc func(*LazyResponseRecorder) error
	check := func(fns ...checkFunc) []checkFunc { return fns }

	hasStatus := func(wantCode int) checkFunc {
		return func(rec *LazyResponseRecorder) error {
	...
	hasContents := func(want string) checkFunc {
		return func(rec *LazyResponseRecorder) error {
			...
			rec.Body.Close()
			if string(body) != want {
				return fmt.Errorf("wrote = %q; want %q", string(body), want)
			}
			return nil
		}
	}
	...
	for _, tt := range [...]struct {
		...
		},
		{
			...
			func(w http.ResponseWriter, r *http.Request) {
				...
				// HeaderMap)
				w.(*LazyResponseRecorder).HeaderMap = nil
				io.WriteString(w, "<html>")
			},
			check(hasHeader("Content-Type", "text/html; charset=utf-8")),
		},
		...
		},
		{
			...
			func(w http.ResponseWriter, r *http.Request) {
				w.(*LazyResponseRecorder).Body = nil
				io.WriteString(w, "hi")
			},
