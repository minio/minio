...
func testAPIPutObjectStreamSigV4Handler(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	bytesDataLen := 1000
	bytesData := generateBytesData(bytesDataLen)
	...
	for i, testCase := range testCases {
		var stream []byte
		stream = append(stream, []byte("1000;chunk-signature=abcd\r\n")...)
		stream = append(stream, testCase.data...)
		stream = append(stream, []byte("\r\n")...)
		stream = append(stream, []byte("0;chunk-signature=abcd\r\n")...)
		stream = append(stream, []byte("\r\n")...)
		...
		req, err := newTestSignedRequest("PUT", getPutObjectURL("", testCase.bucketName, testCase.objectName),
			-1, bytes.NewReader(stream), credentials.AccessKeyID, credentials.SecretAccessKey)
		if err != nil {
			t.Fatalf("Test %d: Failed to create HTTP request for Put Object: <ERROR> %v", i+1, err)
		}
		// t.Errorf("%s\n", strconv.Itoa(testCase.dataLen))
		req.Header.Set("x-amz-decoded-content-length", strconv.Itoa(testCase.dataLen))
		// Since `apiRouter` satisfies `http.Handler` it has a ServeHTTP to execute the logic of the handler.
		// Call the ServeHTTP to execute the handler,`func (api objectAPIHandlers) GetObjectHandler`  handles the request.
		apiRouter.ServeHTTP(rec, req)
		// Assert the response code with the expected status.
