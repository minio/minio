...
func lookupConfigs(s config.Config, objAPI ObjectLayer) {
	...
	if err == nil && dnsURL != "" {
		bootstrapTrace(bsBucketDNSStoreInit)
	...
	if etcdCfg.Enabled {
		bootstrapTrace(bsEtcdStoreInit)
	...
	bootstrapTrace(bsConfigApplyDyn)
...
func loadConfig(objAPI ObjectLayer, data []byte) error {
	bootstrapTrace(bsConfigLoad)
	srvCfg, err := readServerConfig(GlobalContext, objAPI, data)
	if err != nil {
		return err
	}

	bootstrapTrace(bsConfigLookup)
