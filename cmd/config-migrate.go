...
func Save(configFile string, data interface{}) error {
	return quick.SaveConfig(data, configFile, srvCtx.EtcdClient)
}

// Load config from backend
func Load(configFile string, data interface{}) (quick.Config, error) {
	return quick.LoadConfig(configFile, srvCtx.EtcdClient, data)
}

// GetVersion gets config version from backend
func GetVersion(configFile string) (string, error) {
	return quick.GetVersion(configFile, srvCtx.EtcdClient)
}
...
func migrateV23ToV24() error {
	...
	_, err := quick.LoadConfig(configFile, srvCtx.EtcdClient, cv23)
	if osIsNotExist(err) || osIsPermission(err) {
		return nil
	} else if err != nil {
	...
	if err = quick.SaveConfig(srvConfig, configFile, srvCtx.EtcdClient); err != nil {
...
func migrateV24ToV25() error {
	...
	_, err := quick.LoadConfig(configFile, srvCtx.EtcdClient, cv24)
	if osIsNotExist(err) || osIsPermission(err) {
		return nil
	} else if err != nil {
...
func migrateV25ToV26() error {
	...
	_, err := quick.LoadConfig(configFile, srvCtx.EtcdClient, cv25)
	if osIsNotExist(err) || osIsPermission(err) {
		return nil
	} else if err != nil {
...
func migrateV26ToV27() error {
	...
	_, err := quick.LoadConfig(configFile, srvCtx.EtcdClient, srvConfig)
	if osIsNotExist(err) || osIsPermission(err) {
		return nil
	} else if err != nil {
	...
	defer func() {
		if err == nil {
			if srvCtx.EtcdClient != nil {
				deleteKeyEtcd(GlobalContext, srvCtx.EtcdClient, configFile)
			} else {
