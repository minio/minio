...
func migrateConfigToMinioSys(objAPI ObjectLayer) (err error) {
	globalTrace.Publish(bootstrapTrace("migrate config to .minio.sys/config/config.json"))
...
func migrateMinioSysConfig(objAPI ObjectLayer) error {
	globalTrace.Publish(bootstrapTrace("migrate .minio.sys/config/config.json to latest version"))
	// Construct path to config.json for the given bucket.
	configFile := path.Join(minioConfigPrefix, minioConfigFile)
...
func migrateMinioSysConfigToKV(objAPI ObjectLayer) error {
	globalTrace.Publish(bootstrapTrace("migrate config to KV style"))
	configFile := path.Join(minioConfigPrefix, minioConfigFile)
