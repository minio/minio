...
// RemoveUploadIDFromCache notifies all the MinIO nodes to remove the
// given uploadID from cache
func (sys *NotificationSys) RemoveUploadIDFromCache(ctx context.Context, uploadID string) {
	ng := WithNPeers(len(sys.peerClients))
	for idx, client := range sys.peerClients {
		if client == nil {
			continue
		...
		client := client
		ng.Go(ctx, func() error {
			return client.RemoveUploadIDFromCache(ctx, uploadID)
		}, idx, *client.host)
	}
	for _, nErr := range ng.Wait() {
		reqInfo := (&logger.ReqInfo{}).AppendTags("peerAddress", nErr.Host.String())
