...
// SensorTempHandler - returns sensor temp.
func (s *peerRESTServer) SensorTempHandler(w http.ResponseWriter, r *http.Request) {
	if !s.IsValid(w, r) {
		s.writeErrorResponse(w, errors.New("Invalid request"))
		return
	}

	ctx := newContext(r, w, "SensorTemp")
	info := getEndpointsSensorTemp(globalEndpoints, r)
...
func registerPeerRESTHandlers(router *mux.Router) {
	...
	subrouter.Methods(http.MethodPost).Path(SlashSeparator + peerRESTSensorTemp).HandlerFunc(httpTraceHdrs(server.SensorTempHandler))
