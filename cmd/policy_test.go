	...
	p := &policy.BucketPolicy{
		...
		Statements: []policy.BPStatement{
			policy.NewBPStatement("",
				...
				policy.NewResourceSet(policy.NewResourceS3("mybucket")),
			...
			policy.NewBPStatement("",
				...
				policy.NewResourceSet(policy.NewResourceS3("mybucket/myobject*")),
				condition.NewFunctions(),
			),
		},
	}
	...
	return []miniogopolicy.Statement{
		{
			...
			Resources: set.CreateStringSet(policy.NewResourceS3(bucketName).String()),
			Actions:   set.CreateStringSet("s3:GetBucketLocation", "s3:ListBucket"),
		},
		{
			Effect:    string(policy.Allow),
			Principal: miniogopolicy.User{AWS: set.CreateStringSet("*")},
			Resources: set.CreateStringSet(policy.NewResourceS3(bucketName + "/" + prefix).String()),
