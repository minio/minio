...
func (s *xlSets) ListObjectsVersions(ctx context.Context, bucket, prefix, delimiter, keyMarker, versionIDMarker string, maxKeys int) (result ListObjectsVersionsInfo, err error) {
	...
	for i := 0; i < maxKeys; {
		...
		var err error
		objVersions, err = s.getHashedSet(walkResult.entry).getObjectVersions(ctx, bucket, walkResult.entry, versionIDMarker, maxKeys-i)
		if err != nil {
