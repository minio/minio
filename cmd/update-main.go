...
// minioVersionToReleaseTime - parses a standard official released
// Minio version string. An Official binary's version string is the
// release time formatted with RFC3339 (but without the timezone) -
// see minioReleaseTagTimeLayout for the precise format.
func minioVersionToReleaseTime(version string) (releaseTime time.Time, err error) {
	return time.Parse(time.RFC3339, version)
}

// releaseTimeToReleaseTag - converts a release time to an official
// Minio release tag.
...
//
// Any change here should be discussed by opening an issue at https://github.com/minio/minio/issues.
