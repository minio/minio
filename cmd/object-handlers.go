...
func (api objectAPIHandlers) GetObjectHandler(w http.ResponseWriter, r *http.Request) {
	...
	if objInfo.IsCompressed() {
		...
		startOffset, snappyStartOffset = getCompressedOffsets(objInfo, startOffset)
		snappyLength := length
		length = compressedSize - startOffset
		...
		responseWriter := ioutil.LimitedWriter(w, snappyStartOffset, snappyLength)
		wg.Add(1) //For closures.
		go func() {
			defer wg.Done()
			// Finally, writes to the client.
...
func (api objectAPIHandlers) PutObjectPartHandler(w http.ResponseWriter, r *http.Request) {
	...
	isCompressed := false
	if !hasServerSideEncryptionHeader(r.Header) && objectAPI.IsCompressionSupported() && isCompressible(r.Header, object) {
		pipeReader, pipeWriter = io.Pipe()
		snappyWriter := snappy.NewWriter(pipeWriter)
		actualReader, err := hash.NewReader(reader, size, md5hex, sha256hex, actualSize)
		if err != nil {
