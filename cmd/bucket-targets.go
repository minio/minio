...
const (
	defaultHealthCheckDuration = 20 * time.Second
)
...
func (tc *targetClient) setOffline() {
	atomic.StoreInt32(&tc.up, 0)
}

func (tc *targetClient) setOnline() {
...
func (tc *targetClient) healthCheck() {
	for {
		...
		tc.setOnline()
		time.Sleep(time.Duration(tc.healthCheckDuration) * time.Second)
	}
}
