...
func (g *OSS) NewGatewayLayer(creds auth.Credentials) (minio.ObjectLayer, error) {
	...
	if g.host == "" {
		g.host = "https://oss-cn-hangzhou.aliyuncs.com"
	}
...
func (l *ossObjects) MakeBucketWithLocation(ctx context.Context, bucket string, opts minio.BucketOptions) error {
...
func (l *ossObjects) GetObject(ctx context.Context, bucket, key string, startOffset, length int64, writer io.Writer, etag string, opts minio.ObjectOptions) error {
	if strings.HasPrefix(bucket, ".minio") {
...
func (l *ossObjects) DeleteObject(ctx context.Context, bucket, object string, opts minio.ObjectOptions) (minio.ObjectInfo, error) {
	bkt, err := l.Client.Bucket(bucket)
	if err != nil {
		err = ossToObjectError(err, bucket, object)
...
func (l *ossObjects) DeleteObjects(ctx context.Context, bucket string, objects []minio.ObjectToDelete, opts minio.ObjectOptions) ([]minio.DeletedObject, []error) {
...
func (l *ossObjects) AbortMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, opts minio.ObjectOptions) error {
	bkt, err := l.Client.Bucket(bucket)
	if err != nil {
		logger.LogIf(ctx, err)
...
func (l *ossObjects) SetBucketPolicy(ctx context.Context, bucket string, bucketPolicy *policy.Policy) error {
	...
	if err := l.Client.SetBucketACL(bucket, acl); err != nil {
		logger.LogIf(ctx, err)
		return ossToObjectError(err, bucket)
	}
