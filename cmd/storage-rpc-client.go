...
func newRPCClient(networkPath string) (StorageAPI, error) {
	...
	rpcClient := newAuthClient(rpcAddr, rpcPath, cred)
...
func (n networkStorage) MakeVol(volume string) error {
	...
	reply := GenericReply{}
	args := GenericVolArgs{GenericArgs{n.rpcClient.token}, volume}
...
func (n networkStorage) ListVols() (vols []VolInfo, err error) {
	...
	err = n.rpcClient.Call("Storage.ListVolsHandler", &GenericArgs{n.rpcClient.token}, &ListVols)
	if err != nil {
		return nil, err
	}
...
func (n networkStorage) AppendFile(volume, path string, buffer []byte) (err error) {
	...
	if err = n.rpcClient.Call("Storage.AppendFileHandler", &AppendFileArgs{
		GenericArgs: GenericArgs{n.rpcClient.token},
		Vol:         volume,
		Path:        path,
		Buffer:      buffer,
	}, &reply); err != nil {
