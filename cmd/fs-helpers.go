...
func fsRemoveFile(filePath string) error {
	if filePath == "" {
		return errInvalidArgument
	}
	err := os.Remove(preparePath(filePath))
	if err != nil {
...
func fsRemoveAll(dirPath string) error {
	if dirPath == "" {
		return errInvalidArgument
	}
...
func fsRemoveDir(dirPath string) error {
	if dirPath == "" {
		return errInvalidArgument
	}
	err := os.Remove(preparePath(dirPath))
	if err != nil {
		if os.IsNotExist(err) {
			return errVolumeNotFound
		} else if isSysErrNotEmpty(err) {
...
func fsMkdir(dirPath string) error {
	if dirPath == "" {
		return errInvalidArgument
	}
	err := os.Mkdir(preparePath(dirPath), 0777)
	if err != nil {
		if os.IsExist(err) {
...
func fsRenameFile(sourcePath, destPath string) error {
	if err := mkdirAll(path.Dir(destPath), 0777); err != nil {
...
func fsDeleteFile(basePath, deletePath string) error {
	...
	if err := fsDeleteFile(basePath, path.Dir(deletePath)); err != nil {
		return err
	}
	return nil
}
