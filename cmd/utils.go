...
// GetMinMaxTimeInfo - return time infos having min and max current time in given slice
func GetMinMaxTimeInfo(times []madmin.TimeInfo) (min madmin.TimeInfo, max madmin.TimeInfo) {
	min = times[0]
	max = times[0]
	for _, t := range times {
		if len(t.Error) > 0 {
		...
		if t.CurrentTime.After(max.CurrentTime) {
			max = t
		}

		if t.CurrentTime.Before(min.CurrentTime) {
			min = t
		}
	}
	return min, max
...
// AnalyzeTimeInfo - gets time from all nodes and returns a
// Sys Error if there is a significant deviation among them.
func AnalyzeTimeInfo(ctx context.Context) madmin.SysErrors {
	localTimeInfo := madmin.GetTimeInfo(ctx, globalLocalNodeName)
	...
	if diff > 10*time.Second {
		return madmin.SysErrors{
			...
			Errors: []string{
				fmt.Sprintf("Time mismatch on servers. Max time %v on %s is away from min time %v on %s by %v",
					max.CurrentTime, max.Addr, min.CurrentTime, min.Addr, diff),
			},
		}
	}
