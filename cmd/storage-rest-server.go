...
func (s *storageRESTServer) ScannerAndGetDataUsageHandler(w http.ResponseWriter, r *http.Request) {
	if !s.IsValid(w, r) {
		return
	}
	...
	usageInfo, err := s.storage.ScannerAndGetDataUsage(r.Context(), cache)
	if err != nil {
...
// operations, such as bitrot verification or data usage scannering.
...
func registerStorageRESTHandlers(router *mux.Router, endpointServerPools EndpointServerPools) {
	for _, ep := range endpointServerPools {
		for _, endpoint := range ep.Endpoints {
			...
			subrouter.Methods(http.MethodPost).Path(storageRESTVersionPrefix + storageRESTMethodScannerAndGetDataUsage).HandlerFunc(httpTraceHdrs(server.ScannerAndGetDataUsageHandler))
