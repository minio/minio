...
func (s *storageRESTServer) WalkHandler(w http.ResponseWriter, r *http.Request) {
	...
	fch, err := s.storage.Walk(r.Context(), volume, dirPath, markerPath, recursive, leafFile, readMetadata, r.Context().Done())
	if err != nil {
		s.writeErrorResponse(w, err)
		return
	}
...
func (s *storageRESTServer) ListDirHandler(w http.ResponseWriter, r *http.Request) {
	...
	entries, err := s.storage.ListDir(r.Context(), volume, dirPath, count, leafFile)
	if err != nil {
		s.writeErrorResponse(w, err)
		return
	}
...
func (s *storageRESTServer) DeletePrefixesHandler(w http.ResponseWriter, r *http.Request) {
	...
	errs, err := s.storage.DeletePrefixes(r.Context(), volume, prefixes)
	done(nil)
...
func (s *storageRESTServer) VerifyFile(w http.ResponseWriter, r *http.Request) {
	...
	err = s.storage.VerifyFile(r.Context(), volume, filePath, size, BitrotAlgorithmFromString(algoStr), hash, int64(shardSize))
	done(nil)
