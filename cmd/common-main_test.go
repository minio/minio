...
func Test_subPathFromRedirectURL(t *testing.T) {
	type args struct {
		redirectURL string
	}
	tests := []struct {
	...
	}{
		{
			name: "no sub-path",
			args: args{
				redirectURL: "http://console.com:9090",
			},
			want: "",
		},
		{
			name: "Basic sub-path",
			args: args{
				redirectURL: "http://console.com:9090/subpath",
			},
			want: "/subpath/",
		},
		{
			name: "Basic sub-path ending with slash",
			args: args{
				redirectURL: "http://console.com:9090/subpath/",
			},
			want: "/subpath/",
		...
		{
			name: "empty redirect empty subpath",
			args: args{
				redirectURL: "",
			},
			want: "",
		...
		{
			name: "invalid redirect address",
			args: args{
				redirectURL: "ht tp:/console.com:9090/subpath",
			},
			want: "",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := subPathFromRedirectURL(tt.args.redirectURL); got != tt.want {
				t.Errorf("subpathFromRedirectURL() = %v, want %v", got, tt.want)
			}
		})
	}
}
