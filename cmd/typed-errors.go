...
// error returned by IAM when a builtin IDP command is used when they could mean
// to use an LDAP command for groups.
var errNoSuchGroupLDAPWarn = errors.New("Specified group does not exist. If you meant a group in LDAP please use command under `mc idp ldap`")
