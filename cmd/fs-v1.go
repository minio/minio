...
// ScannerAndGetDataUsage returns data usage stats of the current FS deployment
func (fs *FSObjects) ScannerAndGetDataUsage(ctx context.Context, bf *bloomFilter, updates chan<- DataUsageInfo) error {
	// Load bucket totals
	...
	for _, b := range buckets {
		...
		cache, err := fs.scannerBucket(ctx, b.Name, bCache)
		select {
		case <-ctx.Done():
			return ctx.Err()
		default:
		...
		if cache.root() == nil {
			if intDataUpdateTracker.debug {
				logger.Info(color.Green("ScannerAndGetDataUsage:") + " No root added. Adding empty")
			}
			cache.replace(cache.Info.Name, dataUsageRoot, dataUsageEntry{})
		}
		if cache.Info.LastUpdate.After(bCache.Info.LastUpdate) {
			if intDataUpdateTracker.debug {
				logger.Info(color.Green("ScannerAndGetDataUsage:")+" Saving bucket %q cache with %d entries", b.Name, len(cache.Cache))
			}
		...
		if intDataUpdateTracker.debug {
			logger.Info(color.Green("ScannerAndGetDataUsage:")+" Saving totals cache with %d entries", len(totalCache.Cache))
		}
...
// scannerBucket scanners a single bucket in FS mode.
// The updated cache for the bucket is returned.
// A partially updated bucket may be returned.
func (fs *FSObjects) scannerBucket(ctx context.Context, bucket string, cache dataUsageCache) (dataUsageCache, error) {
	// Get bucket policy
	// Check if the current bucket has a configured lifecycle policy
	lc, err := globalLifecycleSys.Get(bucket)
	if err == nil && lc.HasActiveRules("", true) {
		if intDataUpdateTracker.debug {
			logger.Info(color.Green("scannerBucket:") + " lifecycle: Active rules found")
		}
		cache.Info.lifeCycle = lc
	}

	// Load bucket info.
	cache, err = scannerDataFolder(ctx, fs.fsPath, cache, func(item scannerItem) (sizeSummary, error) {
		...
		if err != nil && !osIsNotExist(err) {
			if intDataUpdateTracker.debug {
				logger.Info(color.Green("scannerBucket:")+" object return unexpected error: %v/%v: %w", item.bucket, item.objectPath(), err)
			}
		...
		if fiErr != nil {
			if intDataUpdateTracker.debug {
				logger.Info(color.Green("scannerBucket:")+" object path missing: %v: %w", item.Path, fiErr)
			}
			return sizeSummary{}, errSkipFile
		}
