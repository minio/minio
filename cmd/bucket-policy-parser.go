...
var sentinelBucketPolicy = policy.BucketAccessPolicy{Version: "2012-10-17"}
...
func isValidPrincipals(principal policy.User) (err error) {
	...
	unsupportedPrincipal := principal.AWS.Difference(set.CreateStringSet([]string{"*"}...))
	if !unsupportedPrincipal.IsEmpty() {
		// Minio does not support or implement IAM, "*" is the only valid value.
		// Amazon s3 doc on principals:
