...
func rawToFileInfo(ri RawFileInfo, bucket, object string, readData, inclFreeVers, allParts bool) (FileInfo, error) {
	var xl xlMetaV2
		...
		for i, disk := range disks {
			...
			go func(i int, disk StorageAPI) {
				...
				if opts.VersionID != "" {
					...
					mu.Unlock()
					if err == nil {
						fi, err = rawToFileInfo(ri, bucket, object, readData, opts.InclFreeVersions, true)
						mu.Lock()
						metaArr[i], errs[i] = fi, err
						mu.Unlock()
					}
