...
const (
	...
	EnvUsernameSearchFilter = "MINIO_IDENTITY_LDAP_USERNAME_FILTER"
...
func (l *Config) bind(conn *ldap.Conn, username, password string) error {
	var failed bool
	for _, usernameFormat := range l.UsernameFormats {
...
func (l *Config) Bind(username, password string) ([]string, error) {
	...
	if err = l.bind(conn, username, password); err != nil {
		return nil, err
	}
