...
func TestPosixReadFile(t *testing.T) {
	...
	testCases := []struct {
		...
		// Whole path is 1024 characters long, success case. - 3
		{
		...
		// Object is a directory. - 4
		{
			volume, "object-as-dir",
			0, 5, nil, errIsNotRegular},
		// One path segment length is > 255 chars long. - 5
		{
		...
		// Path length is > 1024 chars long. - 6
		{
			...
			0, 5, nil, errFileNameTooLong},
		// Buffer size greater than object size. - 7
		{
		...
		// Reading from an object but buffer size greater. - 9
		{
			volume, "myobject",
			7, 8,
			[]byte("world"),
