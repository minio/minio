...
func (api objectAPIHandlers) PutObjectPartHandler(w http.ResponseWriter, r *http.Request) {
	...
	pi, local := globalSiteReplicationSys.getPeerForUpload(ctx, uploadID)
	if !local {
		globalSiteReplicationSys.proxyUploadPart(ctx, w, r, bucket, object, uploadID, partID, actualSize, md5hex, sha256hex, pi, opts)
		return
	}
...
func (api objectAPIHandlers) CompleteMultipartUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	pi, local := globalSiteReplicationSys.getPeerForUpload(ctx, uploadID)
	if !local {
		globalSiteReplicationSys.proxyCompleteMultipart(ctx, w, r, bucket, object, uploadID, complMultipartUpload.Parts, pi, opts, writeErrorResponseWithoutXMLHeader)
		return
	}
...
func (api objectAPIHandlers) AbortMultipartUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	pi, local := globalSiteReplicationSys.getPeerForUpload(ctx, uploadID)
	if !local {
		globalSiteReplicationSys.proxyAbortMultipart(ctx, w, r, bucket, object, uploadID, pi)
		return
	}
...
func (api objectAPIHandlers) ListObjectPartsHandler(w http.ResponseWriter, r *http.Request) {
	...
	pi, local := globalSiteReplicationSys.getPeerForUpload(ctx, uploadID)
	if !local {
		globalSiteReplicationSys.proxyListObjectParts(ctx, w, r, bucket, object, uploadID, partNumberMarker, maxParts, encodingType, pi)
		return
	}
	opts := ObjectOptions{}
...
func (api objectAPIHandlers) CopyObjectPartHandler(w http.ResponseWriter, r *http.Request) {
	...
	pi, local := globalSiteReplicationSys.getPeerForUpload(ctx, uploadID)
	if !local {
		globalSiteReplicationSys.proxyCopyObjectPart(ctx, w, r, srcBucket, srcObject, dstBucket, dstObject, uploadID, partID, startOffset, length, pi, srcOpts, dstOpts)
		return
	}
	...
	if isRemoteCopyRequired(ctx, srcBucket, dstBucket, objectAPI) {
		...
		partInfo, _, err := core.PutObjectPart(ctx, dstBucket, dstObject, uploadID, partID, gr, length, "", "", dstOpts.ServerSideEncryption)
		if err != nil {
			writeErrorResponse(ctx, w, toAPIError(ctx, err), r.URL)
			return
		}
