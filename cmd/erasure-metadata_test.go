...
func TestAddObjectPart(t *testing.T) {
	...
	for _, testCase := range testCases {
		if testCase.expectedIndex > -1 {
			...
			fi.AddObjectPart(testCase.partNum, "part."+partNumString, "etag."+partNumString, int64(testCase.partNum+humanize.MiByte), ActualSize)
		}
...
func TestObjectToPartOffset(t *testing.T) {
	...
	for _, partNum := range []int{1, 2, 4, 5, 7} {
		...
		fi.AddObjectPart(partNum, "part."+partNumString, "etag."+partNumString, int64(partNum+humanize.MiByte), ActualSize)
	}
