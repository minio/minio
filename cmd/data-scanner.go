...
func runDataScanner(ctx context.Context, objAPI ObjectLayer) {
	...
	for {
		...
		case <-scannerTimer.C:
			...
			logger.LogIf(ctx, err)
			err = objAPI.ScannerAndGetDataUsage(ctx, bf, results)
			close(results)
			logger.LogIf(ctx, err)
			if err == nil {
				// Store new cycle...
...
// scannerDataFolder will scanner the basepath+cache.Info.Name and return an updated cache.
// The returned cache will always be valid, but may not be updated from the existing.
// Before each operation sleepDuration is called which can be used to temporarily halt the scanner.
// If the supplied context is canceled the function will return at the first chance.
func scannerDataFolder(ctx context.Context, basePath string, cache dataUsageCache, getSize getSizeFn) (dataUsageCache, error) {
	t := UTCNow()
	...
	if s.dataUsageScannerDebug {
		console.Debugf(logPrefix+"Start scannering. Bloom filter: %v %s\n", s.withFilter != nil, logSuffix)
	}
