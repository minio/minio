...
func registerAdminRouter(router *mux.Router, enableConfigOps, enableIAMOps bool) {
	...
	for _, adminVersion := range adminVersions {
		...
		if enableIAMOps {
			...
			adminRouter.Methods(http.MethodPost).Path(adminVersion+"/edit-service-account").HandlerFunc(httpTraceHdrs(adminAPI.EditServiceAccount)).Queries("accessKey", "{accessKey:.*}")
