...
func getOpName(name string) (op string) {
	...
	op = strings.Replace(op, "net/http.Handler.ServeHTTP-fm", "generic", 1)
	...
	op = strings.Replace(op, "(*storageRESTServer)", "storage", 1)
	op = strings.Replace(op, "(*peerRESTServer)", "peer", 1)
	op = strings.Replace(op, "(*lockRESTServer)", "lock", 1)
