...
func runGetObjectBenchmark(b *testing.B, obj ObjectLayer, objSize int) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		buffer, _, err := obj.GetObject(bucket, "object"+strconv.Itoa(i%10), 0, int64(objSize)-1)
		if err != nil {
			b.Error(err)
		}
		_, err = io.Copy(ioutil.Discard, buffer)
		if err != nil {
			b.Error(err)
		}
	}
...
func runGetObjectBenchmarkParallel(b *testing.B, obj ObjectLayer, objSize int) {
	...
	b.RunParallel(func(pb *testing.PB) {
		i := 0
		for pb.Next() {
			objReader, _, err := obj.GetObject(bucket, "object"+strconv.Itoa(i), 0, int64(objSize)-1)
			if err != nil {
				b.Error(err)
			}
