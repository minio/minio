...
type ListVersionsResponse struct {
	...
	CommonPrefixes      []CommonPrefix // When response is truncated (the IsTruncated element value in the response
	...
	MaxKeys     int
	IsTruncated bool // A flag that indicates whether or not ListObjects returned all of the results
...
type ListObjectsResponse struct {
	...
	Delimiter    string
	EncodingType string   `xml:"EncodingType,omitempty"`
	Contents     []Object // When response is truncated (the IsTruncated element value in the response
	// is true), you can use the key name in this field as marker in the subsequent
	// request to get next set of objects. Server lists objects in alphabetical
	// order Note: This element is returned only if you have delimiter request parameter
	// specified. If response does not include the NextMaker and it is truncated,
...
type ListPartsResponse struct {
	...
	StorageClass string
	Parts        []Part `xml:"Part"` // The class of storage used to store the object.
	// List of parts.
...
type ListMultipartUploadsResponse struct {
	...
	Prefix             string
	EncodingType       string   `xml:"EncodingType,omitempty"`
	Uploads            []Upload `xml:"Upload"` // List of pending uploads.

	CommonPrefixes []CommonPrefix // Delimed common prefixes.
...
type Object struct {
	...
	StorageClass string
	Key          string
	Size         int64 // UserMetadata user-defined metadata
