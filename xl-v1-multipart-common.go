...
func isIncompleteMultipart(storage StorageAPI, objectPath string) bool {
	_, err := storage.StatFile(minioMetaBucket, path.Join(objectPath, uploadsJSONFile))
	if err != nil {
		if err == errFileNotFound {
			return false
		}
		errorIf(err, "Unable to access "+path.Join(objectPath, uploadsJSONFile))
		return false
	}
	return true
}
...
func listLeafEntries(storage StorageAPI, prefixPath string) (entries []string, err error) {
	if isIncompleteMultipart(storage, prefixPath) {
...
outerLoop:
	for {
		...
		if objInfo.IsDir {
			...
			// entries are empty.
			entries, err = listLeafEntries(xl.getRandomDisk(), objInfo.Name)
			if err != nil {
		...
		if len(entries) > 0 {
			...
			for _, entry := range entries {
				if xl.isObject(minioMetaBucket, path.Join(objInfo.Name, entry)) {
					objInfo.Name = path.Join(objInfo.Name, entry)
