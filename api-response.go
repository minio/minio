...
// generates an ListObjects response for the said bucket with other enumerated options.
func generateListObjectsV2Response(bucket, prefix, marker, delimiter string, maxKeys int, resp ListObjectsInfo) ListObjectsV2Response {
	var contents []Object
	var prefixes []CommonPrefix
	var owner = Owner{}
	...
	data.Prefix = prefix
	data.StartAfter = marker
	data.Delimiter = delimiter
	data.MaxKeys = maxKeys
	NextContinuationTokenBytes, err := json.Marshal(&ContinuationToken{prefix, resp.NextMarker, delimiter})
	if err != nil {
		return data // FIXME: Handle error
	}
	data.NextContinuationToken = base64.StdEncoding.EncodeToString(NextContinuationTokenBytes)
	data.IsTruncated = resp.IsTruncated
	for _, prefix := range resp.Prefixes {
		var prefixItem = CommonPrefix{}
		prefixItem.Prefix = prefix
