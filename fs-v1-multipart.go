...
func isIncompleteObject(storage StorageAPI, bucket, prefix string) bool {
	_, err := storage.StatFile(bucket, path.Join(prefix, uploadsJSONFile))
	if err != nil {
		if err == errFileNotFound {
			return false
		}
		errorIf(err, "Unable to access "+path.Join(prefix, uploadsJSONFile))
		return false
	}
	return true
}

// listFSLeafEntries - lists all entries if a given prefixPath is a leaf
// directory, returns error if any - returns empty list if prefixPath
// is not a leaf directory.
func listFSLeafEntries(storage StorageAPI, prefixPath string) (entries []string, err error) {
	if isIncompleteObject(storage, minioMetaBucket, prefixPath) {
...
outerLoop:
	for {
		...
		if fileInfo.Mode.IsDir() {
			...
			// entries are empty.
			entries, err = listLeafEntries(fs.storage, fileInfo.Name)
			if err != nil {
		...
		if len(entries) > 0 {
			...
			for _, entry := range entries {
				if isMultipartObject(fs.storage, minioMetaBucket, path.Join(fileInfo.Name, entry)) {
					fileInfo.Name = path.Join(fileInfo.Name, entry)
...
func (fs fsObjects) isUploadIDExists(bucket, object, uploadID string) bool {
	uploadIDPath := path.Join(mpartMetaPrefix, bucket, object, uploadID)
	_, err := fs.storage.StatFile(minioMetaBucket, path.Join(uploadIDPath, xlMetaJSONFile))
	if err != nil {
		if err == errFileNotFound {
			return false
		}
