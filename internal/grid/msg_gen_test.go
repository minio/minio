...
func TestEncodeDecodeMSS(t *testing.T) {
	v := MSS{}
	...
	m := v.Msgsize()
	if buf.Len() > m {
		t.Log("WARNING: TestEncodeDecodeMSS Msgsize() is inaccurate")
	}
...
func BenchmarkEncodeMSS(b *testing.B) {
	v := MSS{}
	var buf bytes.Buffer
	msgp.Encode(&buf, &v)
...
func BenchmarkDecodeMSS(b *testing.B) {
	v := MSS{}
	var buf bytes.Buffer
	msgp.Encode(&buf, &v)
	b.SetBytes(int64(buf.Len()))
