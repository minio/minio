...
// ciphertext.
func EncryptBytes(ctx context.Context, KMS kms.KMS, plaintext []byte, context kms.Context) ([]byte, error) {
	ciphertext, err := Encrypt(ctx, KMS, bytes.NewReader(plaintext), context)
	if err != nil {
		return nil, err
	}
...
// provided.
func DecryptBytes(ctx context.Context, KMS kms.KMS, ciphertext []byte, context kms.Context) ([]byte, error) {
	plaintext, err := Decrypt(ctx, KMS, bytes.NewReader(ciphertext), context)
	if err != nil {
		return nil, err
	}
...
// The same context must be provided when decrypting the
// ciphertext.
func Encrypt(ctx context.Context, KMS kms.KMS, plaintext io.Reader, kmsCtx kms.Context) (io.Reader, error) {
	algorithm := sio.AES_256_GCM
	if !fips.Enabled && !sioutil.NativeAES() {
		algorithm = sio.ChaCha20Poly1305
	}

	key, err := KMS.GenerateKey(ctx, "", kmsCtx)
	if err != nil {
		return nil, err
	}
...
// The same context that have been used during encryption must be
// provided.
func Decrypt(ctx context.Context, KMS kms.KMS, ciphertext io.Reader, kmsCtx kms.Context) (io.Reader, error) {
	const (
		MaxMetadataSize = 1 << 20 // max. size of the metadata
		Version         = 1
	)
	...
	key, err := KMS.DecryptKey(ctx, metadata.KeyID, metadata.KMSKey, kmsCtx)
	if err != nil {
		return nil, err
	}
