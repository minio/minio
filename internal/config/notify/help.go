...
var (
	...
	HelpAMQP = config.HelpKVS{
		...
		config.HelpKV{
			...
			Description: "exclusive queue when set to 'on', only one consumer can be created.",
			Optional:    true,
			Type:        "on|off",
		},
		config.HelpKV{
			Key:         target.AmqpClientTLSCert,
			Description: "path to client certificate for AMQP auth",
			Optional:    true,
			Type:        "path",
			Sensitive:   true,
		},
		config.HelpKV{
			...
			Description: "path to client key for AMQP auth",
			Optional:    true,
			Type:        "path",
			Sensitive:   true,
		},
