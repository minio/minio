...
If you are using an S3 enabled roll on an EC2 instance for S3 access, MinIO will still require env vars MINIO_ACCESS_KEY and MINIO_SECRET_KEY to be set for its internal use. These may be set to any value which meet the length requirements. Access key length should be at least 3, and secret key length at least 8 characters.
```
