...
```sh
$ mc admin config set myminio/ notify_amqp:1 enable="on" exchange="bucketevents" exchange_type="fanout" mandatory="false" no_wait="false"  url="amqp://myuser:mypassword@localhost:5672" auto_deleted="false" delivery_mode="0" durable="false" internal="false" routing_key="bucketlogs"
```
...
```sh
$ mc admin config set myminio notify_mqtt:1 enable="on" broker="tcp://localhost:1883" password="" queue_dir="" queue_limit="0" reconnect_interval="0s"  keep_alive_interval="0s" qos="1" topic="minio" username=""
```
...
```sh
$ mc admin config set myminio notify_elasticsearch:1 enable="on" queue_limit="0"  url="http://127.0.0.1:9200" format="namespace" index="minio_events" queue_dir=""
```
...
```sh
$ mc admin config set myminio/ notify_redis:1 enable="on" address="127.0.0.1:6379" format="namespace" key="bucketevents" password="yoursecret" queue_dir="" queue_limit="0"
```
...
```sh
$ mc admin config set myminio notify_nats:1 enable="on" password="yoursecret" streaming_max_pub_acks_in_flight="10" subject="" address="0.0.0.0:4222"  token="" username="yourusername" ping_interval="0" queue_limit="0" tls="off" streaming_async="on" queue_dir="" streaming_cluster_id="test-cluster" streaming_enable="on"
```
...
```sh
$ mc admin config set myminio notify_postgres:1 enable="on" password="password" port="5432" queue_dir="" connection_string="sslmode=disable" host="127.0.0.1" queue_limit="0"  table="bucketevents" username="postgres" database="minio_events" format="namespace"
```
...
```sh
$ mc admin config set myminio notify_mysql:myinstance enable="on" table="minio_images" database="miniodb" format="namespace" password="" port="3306" queue_dir="" queue_limit="0"  username="root" dsn_string="" host="172.17.0.1"
```
...
```sh
$ mc admin config set myminio notify_kafka:1 enable="on" tls_skip_verify="off"  queue_dir="" queue_limit="0" sasl="off" sasl_password="" sasl_username="" tls_client_auth="0" tls="off" client_tls_cert="" client_tls_key="" brokers="localhost:9092,localhost:9093" topic="bucketevents" version=""
```
...
```sh
$ mc admin config set myminio notify_webhook:1 enable="on" queue_limit="0"  endpoint="http://localhost:3000" queue_dir=""
```
...
```sh
$ mc admin config set myminio notify_nsq:1 enable="on" nsqd_address="127.0.0.1:4150" queue_dir="" queue_limit="0" tls_enable="off" tls_skip_verify="on" topic="minio"
```
